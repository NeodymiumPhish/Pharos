...
enum SettingsForm {
    ...
    /// Insets `content` inside a fresh view on all four sides. All four, not
    /// three: the pane's `fittingSize` is what sets the window's size for this
    /// tab, so an unpinned trailing or bottom edge would size the window to
    /// nothing.
    static func wrap(_ content: NSView, inset: CGFloat = 20) -> NSView {
        ...
        NSLayoutConstraint.activate([
            ...
            wrapper.trailingAnchor.constraint(equalTo: content.trailingAnchor, constant: inset),
