...
enum TagTupleMatcher {
    ...
    struct Index {
        ...
        fileprivate var tupleIds: [String] = []
        /// Slot count per tuple — the test for "complete".
        fileprivate var tupleWidth: [Int] = []
    ...
    static func buildIndex(_ tags: [Tag]) -> Index {
        ...
        for tag in tags {
            ...
            for tuple in tag.tuples {
                ...
                guard !tuple.values.isEmpty else { continue }
                let tupleSlot = index.tupleIds.count
                index.tupleIds.append(tuple.id)
                index.tupleWidth.append(tuple.values.count)
                for (position, value) in tuple.values.enumerated() {
                    let key = TagValueKey(family: value.family, value: value.value)
                    index.slots[key, default: []].append(
                        Index.Slot(tag: tagSlot, tuple: tupleSlot, position: position))
    ...
    static func match(columns: [ColumnDef], rows: [[String?]], index: Index)
        ...
        for (rowIndex, row) in rows.enumerated() {
            ...
            for (column, family) in families.enumerated() {
                ...
                for hit in hits {
                    satisfied[hit.tuple, default: []].insert(hit.position)
                    matchedColumns[hit.tag, default: []].insert(column)
                    touched[hit.tag, default: []].insert(hit.tuple)
                }
            }
            ...
            for (tag, tuples) in touched {
                let complete = tuples
                    .filter { satisfied[$0]?.count == index.tupleWidth[$0] }
                    .map { index.tupleIds[$0] }
                    .sorted()
                matches.append(TagRowMatch(
                    ...
                    matchedTupleIds: tuples.map { index.tupleIds[$0] }.sorted(),
                    solidTupleIds: complete))
            }
