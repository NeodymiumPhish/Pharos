...
class ConnectionSheet: NSViewController {
    ...
    // Stored, not local to `loadView`, so the key view loop wiring in
    // `wireKeyViewLoop()` can reach them.
    ...
    private func wireKeyViewLoop() {
        ...
        portField.nextKeyView = databaseField
        databaseField.nextKeyView = usernameField
        usernameField.nextKeyView = passwordField
        passwordField.nextKeyView = sslPopup
        sslPopup.nextKeyView = defaultSchemaPopup
        defaultSchemaPopup.nextKeyView = testButton
        testButton.nextKeyView = cancelButton
