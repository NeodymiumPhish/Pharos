...
func runTests() {
    ...
    let create = CreateTag(
        ...
        tuples: [NewTagTuple(
            values: [TaggedValue(column: "md5", family: "text",
    ...
    // 2. AddTagTuples and UpdateTag likewise.
    let add = AddTagTuples(tagId: "t1", tuples: [])
    expectTrue(String(data: try! encoder.encode(add), encoding: .utf8)!.contains("\"tagId\""),
               "AddTagTuples emits tagId")
    ...
    {"id":"t1","name":"Suspect infra","colorIndex":2,"note":null,
     ...
     "tuples":[{"id":"u1","values":[{"column":"md5","family":"text",
    ...
    expectEqual(tag.tuples.count, 1, "Tag decodes its tuples")
    expectEqual(tag.tuples[0].values[0].display, "D41D8C", "TaggedValue decodes display")
    expectEqual(tag.tuples[0].originTable, "public.certs", "TagTuple decodes originTable")
