// Standalone runner for TagTupleMatcher — the probe index, the solid/dashed
...
private func value(_ column: String, _ family: String, _ text: String) -> TaggedValue {
    TaggedValue(column: column, family: family,
...
private func tuple(_ id: String, _ values: [TaggedValue]) -> TagTuple {
    TagTuple(id: id, values: values, tupleKey: TupleKey.encode(
                values.map { TagValueKey(family: $0.family, value: $0.value) }) ?? "",
             originConnection: "c1", originTable: "public.certs",
             createdAt: "2026-08-13T00:00:00Z")
}

private func tag(_ id: String, _ tuples: [TagTuple]) -> Tag {
    Tag(id: id, name: id, colorIndex: 0, note: nil,
        createdAt: "2026-08-13T00:00:00Z", updatedAt: "2026-08-13T00:00:00Z", tuples: tuples)
}
...
func runTests() {
    ...
    let index = TagTupleMatcher.buildIndex([md5Tag, pairTag])
    ...
    let matches = TagTupleMatcher.match(columns: columns, rows: rows, index: index)
    ...
    expectEqual(matches[1]?[0].solidTupleIds, ["p1"], "row 1 names the complete tuple")
    ...
    expectEqual(matches[2]?[0].matchedTupleIds, ["p1", "p2"], "both origin tuples are named")
    expectEqual(matches[2]?[0].solidTupleIds, [], "a dashed match names no complete tuple")
    ...
    expectEqual(TagTupleMatcher.match(columns: columns, rows: nullRows, index: index).isEmpty,
    ...
    let emptyIndex = TagTupleMatcher.buildIndex([tag("empty", [tuple("e1", [])])])
    expectEqual(TagTupleMatcher.match(columns: columns, rows: rows, index: emptyIndex).isEmpty,
    ...
    let textPort = TagTupleMatcher.buildIndex(
        [tag("t", [tuple("x", [value("label", "text", "443")])])])
    expectEqual(TagTupleMatcher.match(columns: columns, rows: rows, index: textPort).isEmpty,
                true, "a text 443 does not match a numeric 443")

    // 9. An empty index short-circuits, and so does a result with no column of
    //    any tagged family.
    expectEqual(TagTupleMatcher.match(columns: columns, rows: rows,
                                      index: TagTupleMatcher.buildIndex([])).isEmpty,
                true, "no tags, no work")
    let jsonOnly = [ColumnDef(name: "doc", dataType: "jsonb")]
    expectEqual(TagTupleMatcher.match(columns: jsonOnly, rows: [["x"]], index: index).isEmpty,
    ...
    let twiceIndex = TagTupleMatcher.buildIndex([tag("twice", [
        tuple("d1", [value("a", "text", "dup"), value("b", "text", "dup")])])])
    let twiceRows: [[String?]] = [["dup", "unrelated", "192.0.2.1", "1"]]
    expectEqual(TagTupleMatcher.match(columns: columns, rows: twiceRows,
    ...
    expectEqual(TagTupleMatcher.match(columns: columns, rows: [["d41d8c"]],
    ...
    let tieIndex = TagTupleMatcher.buildIndex([
        tag("a-tag", [tuple("a1", [value("md5", "text", "zzz")])]),
        tag("b-tag", [tuple("b1", [value("subject", "text", "yyy")])]),
    ])
    let tieRows: [[String?]] = [["zzz", "yyy", "192.0.2.1", "1"]]
    let tieMatches = TagTupleMatcher.match(columns: columns, rows: tieRows, index: tieIndex)
