...
#
# The app runs with a scratch HOME (empty store: no connections, no Keychain
# lookups, fresh defaults) so the capture does not depend on this machine's data.
...
APP="${1:?usage: $0 <Pharos.app> [output.json]}"
