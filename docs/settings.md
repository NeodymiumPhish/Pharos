...
| Group inherited tables | On/Off | Off | Tables joined by `INHERITS` read as a partitioned table: the children move under the parent into the Partitions folder and nest as deep as the tree goes, and the parent's row count and size become the whole tree's. This is how partitioning was done before PostgreSQL 10, and PostgreSQL still sees the children as ordinary tables — so with this off they all list flat, as they always have. It carries the folder on its own: the children have left the top level, so Show leaf partitions does not gate it. See [inherited tables](schema-browser.md#inherited-tables). |
...
| Order partitions by | Partition bound, Name, Size | Name | The order inside that Partitions folder. Partition bound reads each partition's own FROM or IN value, so a range-partitioned table reads in date order; MINVALUE comes first, MAXVALUE after the real keys, and DEFAULT last. An inherited tree has no bounds, so it falls back to name order — which for a `YYYYMMDD` name is date order. |
