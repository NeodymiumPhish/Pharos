...
The editor toolbar holds: **Format** (SQL formatter), a **Save** dropdown (Save / Save As… / Export as SQL File…), and — pinned to the right — the **Result Tabs** toggle. The **navigator selector** (a grouped set of four icons over the sidebar), the **connection** pull-down, the **schema** pull-down beside it, **Run** and **Cancel** live in the window toolbar above (see [Query Execution](query-execution.md)); the connection and schema pull-downs both follow the active tab, so the pair reads as "this database, this schema"; choose **View > Customize Toolbar…** to add Format, New Tab, Save Query, a sidebar filter field or a plain sidebar toggle there too. A contextual **Format as SQL list** button appears after pasting a bare list of values (see below).
