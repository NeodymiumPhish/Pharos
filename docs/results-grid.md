...
- Drag the panel's left edge to resize it. The width is shared by every pane and survives a relaunch. The panel keeps the width you chose: the editor gives up the space, down to a narrow floor. Only when the pane is too narrow even for that does the panel give way — the result tabs panel first, then the Query Variables panel, and neither below its own minimum width. Your chosen widths are not overwritten, so they come back when the window widens.
